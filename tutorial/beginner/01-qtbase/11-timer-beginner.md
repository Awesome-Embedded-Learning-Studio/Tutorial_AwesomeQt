...
这里有几个关键点。`setInterval()` 设置的是两次触发之间的时间间隔，单位是毫秒。`start()` 启动定时器，之后每隔指定的间隔，`timeout()` 信号就会被发射一次。当你不需要定时器时，调用 `stop()` 即可停止。

QTimer 还有一个方便的构造方式，可以直接在启动时指定间隔：
...
设置单次模式后，定时器只会在第一次触发后自动停止。这对于「延迟执行」的场景特别有用，比如延迟关闭提示框、延迟加载资源等。

Qt 还提供了一个更简洁的静态方法 `singleShot()`，非常适合一次性任务：
...
重复定时器会一直运行，直到你调用 `stop()` 或者定时器对象被销毁。需要注意的是，定时器的间隔不是绝对精确的。如果事件循环很忙，或者你的槽函数执行时间过长，下一次触发可能会延迟。

你可以通过 `isActive()` 检查定时器是否正在运行，通过 `remainingTime()` 获取距离下一次触发的剩余时间：
...
`QElapsedTimer` 是一个高精度的计时器，用于测量时间间隔，而不是定时触发事件。它通常用于性能测量、超时检测等场景。
...
用自己的话说说：QTimer 和 QElapsedTimer 的本质区别是什么？如果你要做一个「倒计时 10 秒」的功能，你会选择哪个？为什么？
...
**答案参考**：
- QTimer 是「定时触发事件」，会在指定时间后发射信号，适合周期性任务
- QElapsedTimer 是「测量时间间隔」，像一个秒表，适合计算两个时刻之间的时间差
- 做倒计时功能应该用 QTimer，因为它可以每秒触发一次更新显示
- QElapsedTimer 可以用来测量实际经过了多少时间，作为辅助验证
...
定时器用起来很简单，但有些坑真的会让你血压拉满。这里列几个最常见的。

> ⚠️ 坑 #1：定时器对象被提前销毁
> ❌ 错误做法：
> ```cpp
> void setupTimer() {
>     QTimer *timer = new QTimer;  // 没有 parent！
>     timer->setInterval(1000);
>     connect(timer, &QTimer::timeout, []() {
>         qDebug() << "Tick";
>     });
>     timer->start();
>     // 函数结束后 timer 成为野指针，内存泄漏！
> }
> ```
> ✅ 正确做法：
> ```cpp
...
> });
> timer->start();
> // timer 会随 parent 一起销毁
> ```
> 💥 后果：没有 parent 的定时器会内存泄漏，且定时器可能被意外销毁导致程序崩溃
> 💡 一句话记住：定时器必须有 parent，或者手动管理生命周期

> ⚠️ 坑 #2：在错误的线程使用定时器
> ❌ 错误做法：
> ```cpp
> QThread workerThread;
> QTimer *timer = new QTimer;  // 在主线程创建
> timer->setInterval(1000);
> timer->start();
>
> workerThread.start();
...
> timer->moveToThread(&workerThread);  // 移动到工作线程
> timer->setInterval(1000);
>
> // 或者直接在目标线程中创建定时器
> ```
> 💥 后果：定时器在错误的线程不会触发，或者触发后槽函数在错误的线程执行
> 💡 一句话记住：定时器属于哪个线程，就在哪个线程触发
...
> // 耗时操作放到工作线程
> ```
> 💥 后果：槽函数执行期间，整个事件循环被阻塞，导致界面卡死、其他定时器不准
> 💡 一句话记住：定时器槽要快，耗时操作放线程
...
> timer.start();  // 必须启动！
> ```
> 💥 后果：定时器永远不会触发，你会困惑为什么连接了信号槽却什么都没发生
> 💡 一句话记住：配置完间隔后必须调用 start()

---

### 🔲 代码填空

补全以下代码，实现一个每 500 毫秒打印一次计数，打印 5 次后停止的定时器：
...
*(提示：this、setInterval、stop、start)*
...
**答案参考**：
1. 定时器没有设置 parent，会内存泄漏
2. 如果 MyClass 被销毁，定时器仍然存在，但 `this` 已经无效，会崩溃
3. 应该用 `new QTimer(this)` 或者手动管理 timer 的生命周期
4. 正确写法：`QTimer *timer = new QTimer(this);`
...
3. 可以设置倒计时模式，倒计时结束后发出提示

**完成标准：**
你的程序应该能正确响应启动、暂停、重置操作，每秒准时更新显示，倒计时到达零时能触发提示信号。代码结构清晰，定时器管理合理，没有内存泄漏或崩溃风险。

**提示：**
1. 用一个 QTimer 作为计时核心，连接它的 `timeout` 信号到更新显示的槽
2. 需要维护一个「当前秒数」的状态变量，暂停时停止计时器但不重置这个值
3. 倒计时模式可以在更新显示的槽里检查是否到达零
4. QElapsedTimer 可以用来做高精度计时，减少误差累积
