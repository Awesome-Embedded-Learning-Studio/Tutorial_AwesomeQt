...
选择上限值的时候需要考虑几个因素：你的日志产生速率（每秒多少行），你希望用户能向上回溯多少历史记录，以及你能接受的内存开销。几千到一万行是一个比较合理的范围——以每行平均 100 字节计算，一万行大约占 1MB 内存（加上 QTextDocument 的内部结构开销，实际可能在 3-5MB 左右）。对于桌面应用来说这个量级完全不是问题。
