---
title: "3.36 QLCDNumber：液晶数字显示"
description: "如果你用过老式的电子钟、计算器或者汽车仪表盘，一定对那种液晶风格的数字显示不陌生——七段数码管，每个数字由七条线段拼出来，有点工业仪表的味道。"
---
...
上面这段 QSS 会把 QLCDNumber 变成深色背景配绿色数字——典型的仪表盘/工业显示器配色。QLCDNumber 的 QSS 支持相当有限，你不能通过 QSS 改变线段的粗细、圆角、发光效果等。如果需要高度自定义的数码管效果，还是得自己用 QPainter 画，或者用自定义字体（比如搜索 "seven segment font"）。
