---
title: 对象树与所有权源码拆解
description: "Qt 6 对象树的源码级拆解——setParent_helper 三段式（旧父摘除发
ChildRemoved / 改指针 / 新父 append 发 ChildAdded）、~QObject 先 deleteChildren
再摘己、deleteChildren 手写循环防 double-free、deleteLater 投递 DeferredDelete、
moveToThread 有父拒绝背后的对象树同线程硬约束、QWidget willBeWidget 快速分支。"
---

# 现代Qt开发教程（专家篇）1.21——对象树与所有权源码拆解

咱们写 Qt 代码几乎每天都在依赖对象树：`new QLabel("hello", this)`——第二个参数 `this` 是 parent，往后咱们不用管这个 QLabel 的释放，因为它会跟着 `this` 一起被析构。这套"交给 parent 管生命周期"的机制乍看像黑魔法：写完 `new` 就不管了，代码居然不漏内存。这套机制是 QObject 设计的核心，但它背后有一堆值得追问的问题：`setParent` 到底改了什么？为什么析构 parent 会自动 delete 所有 children，而且不会出乱子？为什么 `moveToThread` 对一个有 parent 的对象会直接拒绝？`deleteLater` 和 `delete` 到底差在哪，为什么 Qt 要专门发明它？

这些问题，靠"parent 管理子对象生命周期"这种概括是答不全的，咱们要把所有权机制从源码里挖出来。这一篇把散落在前面几篇里的线索拼起来——[01 篇](./01-qobject-meta-system-expert.md) 咱们见过 `children` 字段存在 `QObjectData` 基类里、`children()` 内联返回它；[02 篇](./02-signal-slot-internals-expert.md) 咱们见过 `~QObject` 的两类连接清理；[07 篇](./07-event-loop-internals-expert.md) 咱们追过 `DeferredDelete` 事件的全链路。入门篇的 [QObject 与元对象系统](../../beginner/01-qtbase/01-qobject-meta-system-beginner.md) 讲过对象树的基本用法，那是知其然，本篇是知其所以然。本篇的边界：拆 `setParent` / 析构级联 `deleteChildren` / `deleteLater` / `moveToThread` 的线程亲和约束，以及 `QWidget` 走的 `willBeWidget` 快速分支；`QWidget` 特有的窗口系统部分（嵌入父 widget 的窗口、平台资源清理）不展开，只到 `willBeWidget` 这个分支点为止。
...
先 `deleteChildren()` 删所有子对象，再 `setParent_helper(nullptr)` 把自己从父对象的 children 列表摘掉。这个顺序乍看不对劲：直觉会想"先把自己从父摘掉，再处理孩子"更干净。但它不是随便定的。子先于父销毁，保证了一个对象析构时，它的整棵子树都已经清理干净，不会留下"父死了子还在"的孤儿。而把自己从父摘掉放在最后，是因为此时自己还在收尾，不能过早断开和父的关系（`deleteChildren` 过程中可能还需要通过 parent 访问一些东西）。
...
对象树是 Qt 管理生命周期的地基：谁负责删谁、能不能搬线程、析构时为什么不能碰兄弟，这三类判断本篇都给了源码层面的依据。本篇全部行号证据按机制归类收在 [code-index · qtbase](../code-index/qtbase/)，带着行号去 `qt_src/qt6.9.1` 翻原文就能核对。QtBase 卷还剩 18（信号槽深水区）与 20 两篇撰写中；要横向换方向的，[02-qtgui 卷](../02-qtgui/) 是绘图的源码拆解，用法面需要补课的回入门篇 [1.1 QObject 与元对象系统](../../beginner/01-qtbase/01-qobject-meta-system-beginner.md)。
