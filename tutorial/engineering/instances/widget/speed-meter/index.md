...
SpeedMeter 是个指针式速度表盘——汽车仪表那种，0 到 220 的弧形刻度，一根指针指到当前值。比 status-led / toggle-switch 难一截的地方不在动画（那套 `value` / `needleAngle` 解耦骨架前面已经趟平），而在于**两套角度体系要在同一个 paintEvent 里自洽**：三角函数的数学角度和 QPainter `drawArc` 的 1/16° 角度不是一套东西，直接混着用就会把弧画反方向、把指针画歪。这件成品把那套换算注释写在 `.cpp` 顶部，能当成「角度体系换算」的样板来读。
...
- **指针式表盘**：背景弧从 225°（左下）顺时针扫 270° 到 -45°（右下），开口朝下，11 条主刻度把量程 10 等分，主刻度间再插 4 条次刻度，针尖指向当前值
...
**② 定一套角度约定，整套 paintEvent 在这套里自洽。**
表盘弧从 225°（左下）顺时针扫 270° 到 -45°（右下），开口朝下：value=0 落在 225°，value=max 落在 -45°。映射函数 `angleForValue(v) = 225 - (v/max) * 270`（`src/speed_meter.cpp:56-61`），角度随 value 增大而减小，对应数学坐标系里从左下经顶部顺时针转到右下。主刻度、次刻度、指针端点全部走这套数学角度 + 三角函数（`x=cx+r*cos(rad)`、`y=cy+r*sin(rad)`），所以它们天然对齐。

**③ 数学角度与 drawArc 角度分别处理，换算注释写死在 .cpp 顶部。**
这是这件成品最容易绊人的地方。`drawArc` 的角度是 1/16°、0°=3 点钟方向、**正值逆时针**——和我们那套「顺时针铺开」的数学约定不是一套东西。直接把数学角度塞进 `drawArc` 会把弧画反方向。解法是背景弧用 `drawArc(rect, 225*16, -270*16)`：起始角 225°、**负扫角** = 顺时针铺开（`src/speed_meter.cpp:201-202`）。换算关系在 `.cpp` 顶部（`src/speed_meter.cpp:23-27`）和 `drawArc` 调用处（`src/speed_meter.cpp:189-191`）都写了注释，改的时候别漏看。

**④ 指针用 save/translate/rotate/restore + QPolygonF，rotate 角度要 +90° 修正。**
指针画成根粗尖细的多边形（`src/speed_meter.cpp:264-269`）。用 `painter.translate(center)` + `painter.rotate(angle)` 比手算两个端点 drawLine 省事，但 `rotate` 的基准是 12 点钟、顺时针为正，而我们的 `needle_angle_` 是数学角度（0°=3 点钟）。所以旋转角 = `needle_angle_ + 90`（`src/speed_meter.cpp:258`）：把数学 0° 摆正到 3 点钟，再 +90° 适配 rotate 的 12 点钟基准。value=0(225°) → rotate 315° 指左下，value=max(-45°) → rotate 45° 指右下，自洽。这套修正同样在注释里写清了（`src/speed_meter.cpp:253-255`）。

**⑤ 动画对象持久成员指针 + stop()/接力，不用 DeleteWhenStopped。**
`needle_anim_` 构造时 `new QPropertyAnimation(this, "needleAngle", this)`，parent=this 由对象树托管（`src/speed_meter.cpp:48`）。每次 setValue 都 `stop() / setStartValue(needle_angle_) / start()` 复用同一个对象（`src/speed_meter.cpp:74-77`）。`DeleteWhenStopped` 那种「停了自动 delete」的写法在 Slider 连续拖动 / Random Jump 狂点时反复 new/delete 还可能悬空，持久指针 + 接力更稳。`setNeedleAngle` 还用 `qFuzzyCompare` 去重防抖（`src/speed_meter.cpp:88`），相邻两帧角度几乎相同就不重绘。

## 4. 怎么读这份 code

按这个顺序读，最快建立心智：

1. **`.cpp` 顶部的角度约定注释**（`src/speed_meter.cpp:23-27`）——先记住「数学角度 0°=3点、逆时针为正；drawArc 是 1/16°、正值逆时针」，后面所有角度代码都在这套里
2. **`angleForValue`**（`src/speed_meter.cpp:56-61`）——value 怎么映射成数学角度，盯 225° 起始、270° 扫角、`max_value_<=0` 的除零保护
3. **`setValue`**（`src/speed_meter.cpp:66-78`）——业务入口，盯 `stop()/setStartValue(needle_angle_)/start()` 这三行接力
4. **`setNeedleAngle`**（`src/speed_meter.cpp:87-94`）——动画每帧回调，纯赋值 + emit + update（对比 status-led 的 setAnimatedColor 同构）
5. **背景弧绘制**（`src/speed_meter.cpp:192-203`）——drawArc 的负扫角顺时针换算，最容易看反
6. **指针绘制**（`src/speed_meter.cpp:252-274`）——save/translate/rotate + 多边形，盯 `rotate(needle_angle_ + 90)` 的 +90 修正
...
| # | 现象 | 原因 | 后果 | 解法 |
|---|---|---|---|---|
| ① | `drawArc` 画出的背景弧方向和刻度/指针铺开方向对不上（弧逆时针、针顺时针） | 把数学角度直接塞进 `drawArc`，忘了它俩角度体系不同（drawArc 正值逆时针、1/16°） | 视觉错乱：弧和刻度各画各的，开口朝向错 | drawArc 用 `起始角 225*16 + 扫角 -270*16`（负扫角=顺时针），换算注释写死在 `.cpp` 顶部（`src/speed_meter.cpp:189-202`） |
| ② | 指针方向整体偏 90°（不指当前值） | 用了 `painter.rotate`，但 `rotate` 基准是 12 点钟、顺时针为正，而 `needle_angle_` 是数学角度（0°=3 点钟） | 指针指向错位，value=0 不指左下 | 旋转角 = `needle_angle_ + 90`（把数学 0° 摆正到 3 点再 +90° 适配 rotate 基准），见 `src/speed_meter.cpp:258` 及上方注释 |
...
| ④ | `needleAngle` 的 WRITE 若错指向 `setValue` | 动画每帧驱动 setValue → setValue 又启动画 → 无限递归 | **栈溢出崩溃** | WRITE 指 `setNeedleAngle`（纯赋值+emit+update），`setValue` 只做业务入口（`include/speed_meter.h:31` + `src/speed_meter.cpp:87`） |
| ⑤ | 窗口缩到极小时弧/轴帽/指针行为未定义或消失 | 半径 `gauge_r`、`needle_len` 等在控件极小时可能 ≤0，`drawArc`/`drawEllipse`/`drawPolygon` 对负尺寸行为未定义 | 控件压扁后绘制异常 | 所有半径 `std::max(1.0, ...)`、`side = std::max(1, min(w,h))` 兜底（`src/speed_meter.cpp:180-187`） |
| ⑥ | 动画反复 new/delete 或指针悬空 | 用 `DeleteWhenStopped`，stop 后对象被 delete、成员指针悬空 | **segfault**（频繁切换时） | 持久成员指针 + parent=this 托管，`stop()/重配/start()` 复用（`src/speed_meter.cpp:46-48`） |
