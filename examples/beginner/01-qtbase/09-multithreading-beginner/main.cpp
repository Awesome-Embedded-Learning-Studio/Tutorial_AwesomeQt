...
void demoMoveToThread() {
    ...
    // 连接信号：线程启动时开始工作
    QObject::connect(thread, &QThread::started, [worker]() { worker->doWork("后台任务1"); });
    ...
    // 连接信号：任务完成后退出线程
    QObject::connect(
        worker, &Worker::workFinished, [thread](const QString& name, const QString& result) {
...
void demoCrossThreadCommunication() {
    ...
    // 正确方式：通过信号槽跨线程通信
    QObject::connect(thread, &QThread::started, [worker]() { worker->doWork("跨线程测试"); });

    // 主线程接收进度信号
    // 这是安全的：Qt 会自动将信号参数拷贝到事件队列
    // 在主线程的事件循环中调用槽函数
    QObject::connect(worker, &Worker::progressChanged, [](const QString& name, int percent) {
        qDebug() << "主线程: " << name << "安全地收到进度更新:" << percent << "%";
    });

    QObject::connect(worker, &Worker::workFinished,
                     [thread](const QString&, const QString&) { thread->quit(); });
