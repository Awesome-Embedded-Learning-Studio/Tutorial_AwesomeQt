 ...
 *     【之后】才执行，故此处捕获的 md.renderer.rules.fence 已是完整链——调用它即得含
 *     .copy/.lang/pre.shiki/.line-numbers-wrapper 的整段 HTML。直接整段包 <details>，内部兄弟链不动。
 *   - 必须整段包（而非只包 <pre>）：VitePress 复制按钮靠 button.copy.nextElementSibling 定位 <pre>，
 *     兄弟链一断复制即失效。
 *   - 代码放在 <details>【外面】（与 <details> 同级）：原生 <details> 关闭时不渲染非 summary 内容；
 *     放外面则代码始终在 DOM 中，纯 CSS :has(details[open]) 控制 display——零 JS、无 JS 也能展开、
 *     @media print 强制显示即完整打印。
 *   - code-group 内的 fence 不折叠（tab 本身已是折叠语义）。
 *
 * 阈值 30：入门层大量 20-30 行可读中等块不误折，只折真正「一大坨」的长块。改这一处常量即可全局调档。
 *
 * AwesomeQt 适配：单语 zh，去掉 tamcpp 的 isEn 双语分支，文案固定中文。
 */
